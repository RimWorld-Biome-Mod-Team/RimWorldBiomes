...
&& x.GetCompProperties<rimworld_biomes.CompProperties_WaterPlant>().allowedBiomes.Contains(map.Biome.defName)
            ...
            foreach (IntVec3 c in map.AllCells)
            {
                ...
                {
                    if (source2.GetCompProperties<rimworld_biomes.CompProperties_WaterPlant>().allowedTiles.Contains(c.GetTerrain(map)))
                    {
                        if (Rand.Chance(source2.GetCompProperties<rimworld_biomes.CompProperties_WaterPlant>().spawnChance))
                        {
                    ...
                    {
                        if (source2.GetCompProperties<rimworld_biomes.CompProperties_WaterPlant>().growNearWater)
                        {
                            bool flag = false;
                            for (int i = c.x - 1; i < c.x + 2; i++)
                            {
                                for (int j = c.z - 1; j < c.z + 2; j++)
                                {
                                    ...
                                    if (temp.InBounds(map) && source2.GetCompProperties<rimworld_biomes.CompProperties_WaterPlant>().allowedTiles.Contains(temp.GetTerrain(map)) && !isWater(temp, map) && !(temp.GetTerrain(map).defName == "Marsh"))
                                    {
                                        if (Rand.Chance(source2.GetCompProperties<rimworld_biomes.CompProperties_WaterPlant>().spawnChance))
                                        {
                                            Plant plant = (Plant)ThingMaker.MakeThing(source2, null);
