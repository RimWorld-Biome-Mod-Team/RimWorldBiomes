    ...
    public class GenStep_WaterAnimal : GenStep
    {
        public override void Generate(Map map){
			...
			List<PawnKindDef> source = (from x in DefDatabase<PawnKindDef>.AllDefsListForReading
									 where x.race.GetCompProperties<rimworld_biomes.CompProperties_WaterAnimal>() != null
									 && x.race.GetCompProperties<rimworld_biomes.CompProperties_WaterAnimal>().allowedBiomes.Contains(map.Biome.defName) select x).ToList<PawnKindDef>();
            //Log.Error("1");
			...
			foreach (IntVec3 c in map.AllCells)
			{
                ...
                if (c.GetEdifice(map) == null && c.GetCover(map) == null && c.GetFirstBuilding(map) == null){
                    if(source2.race.GetCompProperties<rimworld_biomes.CompProperties_WaterAnimal>().allowedTiles.Contains(c.GetTerrain(map))){
                        if(Rand.Chance(source2.race.GetCompProperties<rimworld_biomes.CompProperties_WaterAnimal>().spawnChance)){
